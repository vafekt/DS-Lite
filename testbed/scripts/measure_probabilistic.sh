...
#   T7  fragment-overlap reassembly collision   (per-run flow loss %)
#   T10  off-path DNS-discovery poisoning         (per-run success 0/1)
#   T11  rogue DHCPv6 AFTR hijack                 (per-run success 0/1)
#   T11b transparent hijack (rogue resolver)      (per-run success 0/1)
#
...
# rule-of-three 95% lower bound on the success rate, and for T7 the flow-loss
...
#   N=20 IDS="T7" bash testbed/scripts/measure_probabilistic.sh   # one attack
set -u
C="${CONTAINER_NAME:-ds-lite-lab}"
N="${N:-20}"
IDS="${IDS:-T7 T10 T11 T11b}"
...
for ID in $IDS; do
    ...
    for i in $(seq 1 "$N"); do
        ...
        case "$ID" in
          T7)
            # T7 success = flow loss >= 50%; we also record the loss for mean/SD/CI.
          ...
          *)
            # T10 success = the resolver cached the attacker address (direct state check).
            s=0; printf '%s\n' "$o" | grep -qE 'verdict:[[:space:]]*MATCH' && s=1
            printf '%d %d\n' "$i" "$s" >> "$datf"

...
the ADVERTISE and the REPLY. This option is the surface that T9/T9 (rogue
...
(T9/T9) is a second ADVERTISE/REPLY whose AFTR-Name ≠ `aftr.dslite.example.com`
or whose DNS-server ≠ `::2` (see `attacks/T9/`, `attacks/T9/`).
...
and the public server name resolves to `198.51.100.2`. This is the surface T8
(off-path poisoning) targets and DNS-0x20 defends.

**Recognition rule:** a correct resolution returns `aftr.dslite.example.com →
AAAA 2001:db8:cafe::10`. A poisoned one (T8) returns the attacker address
...
address-type, **not** a threshold). This is the surface T10 (raise/disable an
alarm threshold) and T11 (walk + disclose the bind table) abuse, and that
SNMP_USM defends.
...
shared public IPv4. Every data-plane attack (T1 exhausting the NAT, T2/T4
forging the softwire, T3 reading it, T5 poisoning reassembly) targets this path.
...
the shared `192.0.2.1`. A forged softwire (T2/T4/T5) has outer source `::b41` but
did NOT originate from b4-1's bridge port (see `attacks/T2`, `T4`, `T5`).

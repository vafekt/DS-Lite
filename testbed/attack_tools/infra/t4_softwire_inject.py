#!/usr/bin/python
# T4 - Downstream Softwire Injection (inbound dual of T2) into a subscriber LAN (P2, on-path carrier).
#
...
#
# This is the inbound dual of T2 (T2 forges B4 -> AFTR to impersonate a subscriber to the
# AFTR; T4 forges AFTR -> B4 to inject into the subscriber). Damage: scan and exploit LAN
...
def run(args):
    print(f"[*] T4 - Downstream Softwire Injection (inbound dual of T2) (vantage P2, on-path carrier)")
    ...
    print(f"*** T4: {args.count} unsolicited inbound packet(s) injected into the LAN, "
          f"bypassing the AFTR CGN filter (verify on the B4 LAN capture) ***")
    return 0


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="T4 Downstream Softwire Injection (inbound dual of T2, P2)")
